                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else if (strcmp(cmd, "SEND") == 0)
                            {
                                ...
                                max_receiver_ips = sendMessage(ip, message1, client_list, stats, blocked_struct_list, message_buffer_list, sock_index, count_block_indexes, max_receiver_ips, 0, 0);
                            }
                            ...
                            else if (strcmp(cmd, "BUFFER") == 0)
                            {
                                ...
                                sendBufferedMessages(client_list, stats, message_buffer_list, sock_index, max_receiver_ips, count_block_indexes, blocked_struct_list, index);
                            }
...
int sendBufferedMessages(struct client_details client_list[100], struct client_details stats[5], struct message_details message_buffer_list[5], int sock_index, int max_receiver_ips, int count_block_indexes, struct blocked_details blocked_struct_list[5], int index)
{
    ...
    for (int a = 0; a < 5; a++)
    {
        ...
        if (message_buffer_list[a].ip_receiver != NULL && strcmp(message_buffer_list[a].ip_receiver, logged_in_ip) == 0)
        {
            ...
            if (index == message_buffer_list[a].count)
            {
                sendAcknowledgement("LOGINACK", sock_index);
            }

            if (message_buffer_list[a].messages_list[index] != NULL)
            {

                // char *nextClient = (char *)malloc(sizeof(char) * BUFFER_SIZE);
                // sprintf(nextClient, "%s %d %s", "BUFFER ", (index + 1), message_buffer_list[a].messages_list[index]);
                // printf("nextClient:%s\n", nextClient);

                // printf("index:%d\n", index);
                // printf("message_buffer_list[a].count:%d\n", message_buffer_list[a].count);

                max_receiver_ips = sendMessage(logged_in_ip, message_buffer_list[a].messages_list[index], client_list, stats, blocked_struct_list, message_buffer_list, sock_index, count_block_indexes, max_receiver_ips, 1, (index + 1));

                // message_buffer_list[a].messages_list[index] = '\0';
                // message_buffer_list[a].sender_ip[index] = '\0';
                // count--;
            }
...
int sendMessage(char *ip, char *message1, struct client_details client_list[100], struct client_details stats[5], struct blocked_details blocked_struct_list[5], struct message_details message_buffer_list[5], int sock_index, int count_block_indexes, int max_receiver_ips, int isBuffer, int bufferIndex)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        {
            sender_ip_message = malloc(sizeof(char) * BUFFER_SIZE);
            if (isBuffer == 1)
            {
                sprintf(sender_ip_message, "%s %d ", "BUFFER", bufferIndex);
            }
            strcat(sender_ip_message, client_list[i].ip_addr);
    ...
    else
    {
        ...
        if (isBuffer == 1)
        {
            char *bufferMessage = malloc(strlen(message1) + 1);
            strcpy(bufferMessage, message1);
            char *bufferSenderIp = strtok(bufferMessage, " ");
            char *newMessage = strtok(NULL, " ");

            strcpy(sender_ip, bufferSenderIp);
            successMessage("RELAYED");
            cse4589_print_and_log("msg from:%s, to:%s\n[msg]:%s\n", bufferSenderIp, ip, newMessage);
            endMessage("RELAYED");
        }
        else
        {
